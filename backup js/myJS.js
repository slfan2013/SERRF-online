...
function twoArraysEqual(arr1, arr2) {
...
serrf_wrapper_extrapolate = function(x, y, span_vals = _.range(0.01, 1, 0.01), folds=5, iterations=iter, accuracy=acy,ratio=0.8){
       ...
       for(var i=0; i<mean_abs_error.length;i++){
        ...
        try {
                var yValuesSmoothed = rf2(x, y, cv_weights);
                var y_predicted = cv_index_test.map(idx => yValuesSmoothed[idx])
                if(y_predicted.includes(NaN)){
                  k--;
                }else{
                  ...
                  /*'Create':{
                    ...
                    'action':function(obj){
                      ...
                      tree.edit($node, null, function(node){// check if the node's new name has been taken. If so, delete this node. Otherwise, create a new node, with id 'newname'+Data().
                        ...
                        db = new PouchDB('http://slfan:metabolomics@localhost:5985/abib');
                        db.get(project_id).then(function(doc){
                          ...
                          if(sibling_name.indexOf(nd.text) > -1){
                            ...
                            // reload the tree.
                            var db = new PouchDB('http://slfan:metabolomics@localhost:5985/abib');db.put(doc).then(function(){$scope.load_tree(project_id);$('#project_structure_ibox').children('.ibox-content').removeClass('sk-loading');});
                          }
                  ...
                  "Download":{
                    ...
                    "action":function(obj){
                      ...
                      if($node.id.indexOf("_csv") !== -1){
                        window.open("http://slfan:metabolomics@localhost:5985/abib/"+project_id+"/"+$node.id.replace('+',"%2B").replace("_csv",".csv"))
                      }else if($node.id.indexOf("_xlsx") !== -1){
                        window.open("http://slfan:metabolomics@localhost:5985/abib/"+project_id+"/"+$node.id.replace('+',"%2B").replace("_xlsx",".xlsx"))
                      }else if($node.id.indexOf("_pptx") !== -1){
                        window.open("http://slfan:metabolomics@localhost:5985/abib/"+project_id+"/"+$node.id.replace('+',"%2B").replace("_pptx",".pptx"))
                      }else if($node.id.indexOf("_png") !== -1){
                        window.open("http://slfan:metabolomics@localhost:5985/abib/"+project_id+"/"+$node.id.replace('+',"%2B").replace("_png",".png"))
                      }else if($node.id.indexOf("_svg") !== -1){
                        window.open("http://slfan:metabolomics@localhost:5985/abib/"+project_id+"/"+$node.id.replace('+',"%2B").replace("_svg",".svg"))
                      }else if($node.id.indexOf("_html") !== -1){
                        window.open("http://slfan:metabolomics@localhost:5985/abib/"+project_id+"/"+$node.id.replace('+',"%2B").replace("_html",".html"))
                      }else if($node.id.indexOf("_pdf") !== -1){
                        window.open("http://slfan:metabolomics@localhost:5985/abib/"+project_id+"/"+$node.id.replace('+',"%2B").replace("_pdf",".pdf"))
