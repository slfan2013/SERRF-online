rf = function() {
  ...
  function smooth(xval, yval, weights) {
    ...
    science_stats_rfFiniteReal(xval);
    science_stats_rfFiniteReal(yval);
    science_stats_rfFiniteReal(weights);
    science_stats_rfStrictlyIncreasing(xval);
    ...
    while (++iter <= robustnessIters) {
      ...
      i = -1; while (++i < n) {
        ...
        if (i > 0) {
          science_stats_rfUpdateBandwidthInterval(xval, weights, i, bandwidthInterval);
        }
        ...
        for (var k = ileft; k <= iright; ++k) {
          var xk   = xval[k],
              yk   = yval[k],
              dist = k < i ? x - xk : xk - x,
              w    = science_stats_rfTricube(dist * denom) * robustnessWeights[k] * weights[k],
              xkw  = xk * w;
          sumWeights += w;
          sumX += xkw;
          sumXSquared += xk * xkw;
      ...
      var medianResidual = jStat.median(residuals);
...
function science_stats_rfFiniteReal(values) {
  var n = values.length,
      i = -1;
...
function science_stats_rfStrictlyIncreasing(xval) {
  var n = xval.length,
      i = 0;
...
function science_stats_rfTricube(x) {
...
function science_stats_rfUpdateBandwidthInterval(
  ...
  var nextRight = science_stats_rfNextNonzero(weights, right);
  if ((nextRight < xval.length) && (xval[nextRight] - xval[i]) < (xval[i] - xval[left])) {
    var nextLeft = science_stats_rfNextNonzero(weights, left);
    bandwidthInterval[0] = nextLeft;
    bandwidthInterval[1] = nextRight;
  }
}

function science_stats_rfNextNonzero(weights, i) {
  var j = i + 1;
